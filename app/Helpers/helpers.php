...
function url($path = '')
{
    $baseUrl = $GLOBALS['config']->get('app')['url'];

    // Ensure the path is prepended with a forward slash if it's not empty
    if ($path && $path[0] !== '/') {
        $path = '/' . $path;
    }

    // Return the full URL
    return rtrim($baseUrl, '/') . $path; // rtrim ensures no trailing slash is added to the base URL
}

/**
 * Redirect to a given URL with optional query parameters.
 *
 * @param string $url The URL to redirect to
 * @param array $queryParams (optional) Associative array of query parameters to append to the URL
 * @param int $statusCode HTTP status code (default is 303)
 * @return void
 */
function redirect(string $url, array $queryParams = [], int $statusCode = 303): void
{
