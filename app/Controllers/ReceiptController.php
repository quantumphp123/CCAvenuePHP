...
class ReceiptController
{
    public function downloadPDF(array $params)
    {
        try {
            ...
            $transactionData = (new PaymentProcessor)->getTransactionDetails($orderId);
            ...
            $pdf->Cell(0, 6, $transactionData['transaction']['bank_ref_no'] ? 'Reference No.: ' . $transactionData['transaction']['bank_ref_no'] : 'Order ID: ' . $orderId, 0, 1, 'C');
            $pdf->Ln(5);
            ...
            $pdf->MultiCell(
                ...
                $transactionData['transaction']['city'] . ', ' . $transactionData['transaction']['state'] . "\n" .
                $transactionData['transaction']['zip_code'] . "\n" .
            ...
            $pdf->MultiCell(
                95,
                6,
                'Date: ' . (isset($transactionData['transaction']['transaction_time']) && $transactionData['transaction']['transaction_time']
                    ? date('F j, Y, g:i a', strtotime($transactionData['transaction']['transaction_time']))
                    : date('F j, Y, g:i a')) . "\n" .
                ...
                ($transactionData['transaction']['transaction_fee'] ? 'Transaction Fee: ' . $transactionData['transaction']['transaction_fee'] . "\n" : ''),
                0,
                'L'
            );
            ...
            $pdf->write2DBarcode($GLOBALS['config']->get('app')['url'] . '/success?order_id=' . $orderId, 'QRCODE,L', 15, $pdf->GetY() + 5, 30, 30, $style);

            ob_end_clean();
            $pdf->Output('receipt_' . $orderId . '.pdf', 'D');
            exit;
        } catch (Exception $e) {
    ...
    public function downloadHTML(array $params)
    {
        try {
            ...
            $orderId = htmlspecialchars($params['order_id']);
            ...
            header('Content-Disposition: attachment; filename="receipt_' . $orderId . '.html"');
    ...
    private function generateReceiptHTML(
        ...
        $cardNetworkHtml = '';
        if (!empty($transactionData['transaction']['card_network'])) {
            $cardNetworkHtml = "<span class='text-sm text-gray-500'>({$transactionData['transaction']['card_network']})</span>";
        }
        return <<<HTML
<!DOCTYPE html>
<html lang='en'>
...
<body>
    <div class='container'>
        ...
        <div class='receipt-details'>
            ...
            <p><span class='bold'>Transaction Fee:</span> {$transactionData['transaction']['transaction_fee']} {$transactionData['transaction']['currency_type']}</p>

            <p><span class='bold'>Bank Reference:</span> {$transactionData['transaction']['bank_ref_no']}</p>
            <p><span class='bold'>Transaction ID:</span> {$transactionData['transaction']['payment_id']}</p>
            <p><span class='bold'>Payment Method:</span> {$transactionData['transaction']['payment_method']} {$cardNetworkHtml}</p>
